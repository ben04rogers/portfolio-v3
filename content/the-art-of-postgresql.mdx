...
Three chapters that build a simple dataset (the CIA Factbook retail sales data) and a comparison: twenty lines of Python versus one SQL query that computes the same thing.

### Why PostgreSQL?

PostgreSQL is described as the "world's most advanced Open Source database." Fontaine is a Major Contributor, so this isn't an unbiased endorsement. The case is still strong: rich data types, extensibility, advanced indexing, real concurrency control, and a query planner that gets smarter every release.
...
This is where the "SQL is code" thesis becomes practical. Six chapters on how to actually write SQL in an application context.
...
Variants: partial, covering, functional indexes, plus `fillfactor` and HOT updates for write-heavy tables. Index for both constraints and query patterns, but every index has a write cost.

## Part 4: SQL Toolbox

Ten chapters on the SQL features most ORMs hide from you.
...
F1 world champions example query: one query, all champions since 1950, drivers and constructors, using `GROUPING SETS` and chained CTEs.
...
Six chapters on the rich type system PostgreSQL actually offers.
...
Eight chapters. Fontaine deliberately puts this late: you need to know SQL's capabilities before you can design a model that takes advantage of them.
...
Eight chapters on DML and what actually happens when two writers show up.

### Insert, Update, Delete

`INSERT INTO ... SELECT` is a workhorse for batch loads. `MERGE` (PG 15+) finally gives you upsert-with-delete in one statement. `RETURNING` gives you back the rows you wrote, useful for triggers and ETL.
...
A package of SQL objects (functions, types, operators, indexes) installed with `CREATE EXTENSION`. Fontaine lists the notable ones: `bloom`, `earthdistance`, `hstore`, `ltree`, `pg_trgm`, `PostGIS`, `ip4r`, `citus`, `pg_partman`, `postgres-hll`, `prefix`, `madlib`, `RUM`.

### Auditing with hstore

A trigger that records every change to a row as an `hstore` key/value diff. Useful for audit trails without a separate schema or service.

### Trigrams and Typos with pg_trgm

Fuzzy text matching. `similarity()`, the `%` operator, GIN-indexed.

### Geolocation with ip4r

IP address ranges as a first-class type. IP-to-location lookups with proper indexing.

### Counting Distinct Users with HyperLogLog

Probabilistic distinct counts. Fast, cheap, lossy.
...
SQL is application code. The database is part of your application. Version it, test it, review it. Most of the performance and complexity problems I've seen in production were SQL problems dressed up as application problems.

The four chapters that changed my query writing most: window functions, `GROUPING SETS`, range types with exclusion constraints, `pg_trgm`.
