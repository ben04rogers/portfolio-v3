...
The `vector` type, distance operators (`<=>`, `<->`, `<#>`), IVFFlat and HNSW indexes, recall measurement. Plus a hybrid search example combining vector similarity with trigram fuzzy match using Reciprocal Rank Fusion.
